...
public class JUnit4InstrumentationTestRunner extends InstrumentationTestRunner implements TestSuiteProvider {
    ...
    public void onCreate(Bundle arguments) {
        ...
        String[] apkPaths = { getTargetContextPackageCodePath(), getContextPackageCodePath() };
        ...
        if (arguments != null) {
            ...
            mDebug = getBooleanArgument(arguments,
                                        "debug");
            mJustCount = getBooleanArgument(arguments,
                                            "count");
            mSuiteAssignmentMode = getBooleanArgument(arguments,
                                                      "suiteAssignment");
            ...
            logOnly = getBooleanArgument(arguments,
                                         ARGUMENT_LOG_ONLY);
            mCoverage = getBooleanArgument(arguments,
                                           "coverage");
            ...
            try {
                ...
                Log.e(LOG_TAG,
                      "Invalid delay_msec parameter",
                      e);
            }
        }

        JUnit4TestSuiteBuilder testSuiteBuilder = new JUnit4TestSuiteBuilder(getClass().getName(), getTargetContext().getClassLoader());
        ...
        if (testClassesArg == null) {
            if (mPackageOfTests != null) {
                ...
                Class<?>[] testSuite = getTestSuiteClasses();
                if (testSuite != null) {
            ...
            parseTestClasses(testClassesArg,
                             testSuiteBuilder);
        }
        ...
        junit4 = new JUnitCore();
        junit4.addListener(new JUnit4TestClassAdaptingListener(junit4results, JUnit4TestAdapterCache.getDefault()));

        if (!mSuiteAssignmentMode) {
            performanceResultPrinter = new WatcherResultPrinter(0);
        }

        junit4Request = testSuiteBuilder.build();

        junit4Runner = junit4Request.getRunner();

        mTestCount = junit4Runner.testCount();

        if (performanceResultPrinter != null) {
            performanceResultPrinter.setNumTests(mTestCount);
        }

        if (mSuiteAssignmentMode) {
            junit4results.addListener(new SuiteAssignmentPrinter());
        } else {
            junit4results.addListener(new TestPrinter("TestRunner", false));
            junit4results.addListener(performanceResultPrinter);
        }
    ...
    public void onStart() {
        ...
        if (mJustCount) {
            ...
            try {
                ...
                resultPrinter.print(junit4results,
                                    runTime);
            } catch (Throwable t) {
                ...
                mResults.putString(Instrumentation.REPORT_KEY_STREAMRESULT,
                                   String.format("\nTest results for %s=%s",
                                                 ...
                                                 junit4Runner.getDescription().getDisplayName(),
    ...
    private StringResultPrinterWrapper createStringResultPrinter(PrintStream ps) {
        Class<?>[] declaredClasses = InstrumentationTestRunner.class.getDeclaredClasses();
        for (Class<?> c : declaredClasses) {
            if (c.getName().endsWith("StringResultPrinter")) {
     ...
     * @deprecated Use getAllTestClasses() instead.
     */
    ...
    private void reportEmmaError(String hint,
                                 Exception e) {
    ...
    private class SuiteAssignmentPrinter implements TestListener {
        ...
        public void endTest(Test test) {
            ...
            mTestResult.putString(Instrumentation.REPORT_KEY_STREAMRESULT,
                                  ((Describable) test).getDescription().getClassName() + "#" + ((Describable) test).getDescription().getMethodName() + "\nin " + assignmentSuite + " suite\nrunTime: " + String.valueOf(runTime) + "\n");

            mTestResult.putFloat(REPORT_KEY_RUN_TIME,
                                 runTime);
            mTestResult.putString(REPORT_KEY_SUITE_ASSIGNMENT,
                                  assignmentSuite);
    ...
    private class WatcherResultPrinter implements TestListener, PerformanceResultsWriter {
        ...
        public void addError(Test test,
            ...
            mTestResult.putString(Instrumentation.REPORT_KEY_STREAMRESULT,
                                  String.format("\nError in %s:\n%s",
                                                ((Describable) test).getDescription().getMethodName(),
                                                BaseTestRunner.getFilteredTrace(t)));
        }
